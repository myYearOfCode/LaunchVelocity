...
RSpec.describe Api::V1::UsersController, type: :controller do
  describe "GET #show" do
    ...
    it "returns successfully with valid input" do
      ...
      expect(returned_json.length).to be(14)
      expect(returned_json["id"]).to eq(@valid_user.id)
      expect(returned_json["email"]).to eq(@valid_user.email)
  ...
  describe "GET #index" do
    ...
    it "returns an array of all users" do
      ...
      expect(returned_json[0]["email"]).to eq(@valid_user.email)
      ...
      expect(returned_json[1]["email"]).to eq(@valid_user2.email)
